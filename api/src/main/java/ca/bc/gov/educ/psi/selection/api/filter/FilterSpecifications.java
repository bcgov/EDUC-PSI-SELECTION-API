...
public class FilterSpecifications<E, T extends Comparable<T>> {
    ...
    public void initSpecifications() {
        ...
        map.put(FilterOperation.NOT_EQUAL_OTHER_COLUMN, filterCriteria -> (root, criteriaQuery, criteriaBuilder) -> {
            ...
            throw new PSISelectionAPIRuntimeException("Invalid search criteria provided");
        });
        ...
        map.put(FilterOperation.NOT_EQUAL, filterCriteria -> (root, criteriaQuery, criteriaBuilder) -> {
            if (filterCriteria.getFieldName().contains(".")) {
                ...
                if(splits.length == 2) {
                    ...
                    if(filterCriteria.getConvertedSingleValue() == null) {
                        return criteriaBuilder.isNotNull(root.get(splits[0]).get(splits[1]).get(splits[2]));
                    }else {
        ...
        map.put(FilterOperation.STARTS_WITH, filterCriteria -> (root, criteriaQuery, criteriaBuilder) -> {
            if (filterCriteria.getFieldName().contains(".")) {
                String[] splits = filterCriteria.getFieldName().split("\\.");
                return criteriaBuilder.like(root.join(splits[0]).get(splits[1]), filterCriteria.getConvertedSingleValue() + "%");
            }
            return criteriaBuilder.like(root.get(filterCriteria.getFieldName()), filterCriteria.getConvertedSingleValue() + "%");
        });
        ...
        map.put(FilterOperation.STARTS_WITH_IGNORE_CASE, filterCriteria -> (root, criteriaQuery, criteriaBuilder) -> {
            if (filterCriteria.getFieldName().contains(".")) {
                ...
                return criteriaBuilder.like(
                    criteriaBuilder.lower(root.join(splits[0]).get(splits[1])),
                    filterCriteria.getConvertedSingleValue().toString().toLowerCase() + "%"
                );
            }
            return criteriaBuilder.like(
                criteriaBuilder.lower(root.get(filterCriteria.getFieldName())),
                filterCriteria.getConvertedSingleValue().toString().toLowerCase() + "%"
            );
        });
        ...
        map.put(FilterOperation.NOT_STARTS_WITH, filterCriteria -> (root, criteriaQuery, criteriaBuilder) -> {
            if (filterCriteria.getFieldName().contains(".")) {
                String[] splits = filterCriteria.getFieldName().split("\\.");
                return criteriaBuilder.notLike(root.join(splits[0]).get(splits[1]), filterCriteria.getConvertedSingleValue() + "%");
            }
            return criteriaBuilder.notLike(root.get(filterCriteria.getFieldName()), filterCriteria.getConvertedSingleValue() + "%");
        });
